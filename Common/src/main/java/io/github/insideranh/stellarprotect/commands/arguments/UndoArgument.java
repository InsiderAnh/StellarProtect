...
public class UndoArgument extends StellarArgument {
    ...
    public void onCommand(@NotNull CommandSender sender, String[] arguments) {
        ...
        // Si no se especifica tiempo, usar los últimos 5 minutos por defecto
        if (timeArg == null) {
        ...
        ArgumentsParser.parseUsers(arguments).thenAccept(usersArg -> {
            ...
            executeUndo(player, hashTagsArg.isPreview(), hashTagsArg.isVerbose(), hashTagsArg.isSilent(), databaseFilters);
        });
    }
    ...
    public void executeUndo(Player player, boolean preview, boolean verbose, boolean silent, DatabaseFilters filters) {
        ...
        plugin.getProtectDatabase().countRestoreActions(filters).thenAccept(totalLogs -> {
            ...
            if (!silent) {
                if (verbose) {
                    player.sendMessage(plugin.getLangManager().get("messages.undo.start-verbose").replace("<total>", String.valueOf(totalLogs)).replace("<batch>", String.valueOf(BATCH_SIZE)));
                } else {
                    player.sendMessage(plugin.getLangManager().get("messages.undo.start").replace("<total>", String.valueOf(totalLogs)).replace("<batch>", String.valueOf(BATCH_SIZE)));
                }
            }

            processBatchUndo(player, preview, verbose, silent, filters, 0, totalLogs, BATCH_SIZE, DELAY_TICKS);
        }).exceptionally(error -> {
