...
public class GrowStructureListener implements Listener {
    ...
    public void onTreeGrow(StructureGrowEvent event) {
        if (ActionType.GROW_STRUCTURE.shouldSkipLog(event.getWorld().getName(), event.getSpecies().name())) return;

        event.getBlocks().forEach(block -> LoggerCache.addLog(new PlayerBlockLogEntry(getPlayerId(event.getPlayer()), block, ActionType.GROW_STRUCTURE)));
    }
